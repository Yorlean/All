...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 02-05-2024 a las 17:04:26
...
(10, 9, '2023-08-08', 'credito', 281700, '2023-09-07', 231, 381700, 'S'),
...
(46, 1, 1, '2023-08-06', '2023-08-07', 200000, 'n', 'Completo'),
(47, 1, 2, '2023-08-06', '2023-08-07', 200000, 'n', 'Completo'),
...
(58, 1, 13, '2023-08-09', '2023-08-09', 350000, 'n', 'Completo'),
...
(65, 1, 20, '2023-08-10', '2023-08-10', 370000, 'n', 'Completo'),
...
(72, 1, 21, '2023-08-13', '2023-08-13', 320000, 'n', 'Completo'),
(73, 2, 22, '2023-08-13', '2023-08-15', 300000, 's', 'Completo'),
...
(79, 1, 28, '2023-08-14', '2023-08-14', 270000, 'n', 'Completo'),
(80, 2, 29, '2023-08-15', '2023-08-15', 290000, 's', 'Completo'),
(81, 3, 30, '2023-08-15', '2023-08-17', 300000, 's', 'Completo'),
...
(86, 1, 35, '2023-08-15', '2023-08-15', 320000, 'n', 'Completo'),
...
(8, 'Pamela', '', 'Serna', 'Muñoz', 'cc', '1010654382', 'femenino', 'Calle 53 No 10-60/46, Pis', 'barranquilla', 40, '3225986478', '2024-03-13', 'soltera', 2, 'tecnico', 'nueva eps', 'skandia', 'porvenir', 'BBVA', 'A'),
...
(14, 'Rocio', '', 'Muñoz', 'Gutierrez', 'cc', '1015768903', 'femenino', 'CR 5B #50-49A SUR', 'barranquilla', 37, '3125847512', '2024-03-13', 'soltera', 3, 'tecnico', 'sura', 'porvenir', 'porvenir', 'BBVA', 'A');
...
(5, 'Salsa de tomate frasco', 7865, 7500, 6000, 4084, '2023-06-01', '2024-06-01', 5, 7),
...
(11, 'Salsa de soya frasco', 6655, 6500, 6500, 4079, '2023-08-20', '2024-09-20', 5, 7),
(12, 'Salsa mayonesa frasco', 6655, 6500, 8000, 4079, '2023-08-20', '2024-09-20', 5, 7),
(13, 'Salsa rosada frasco', 9075, 8500, 7600, 4079, '2023-08-20', '2024-09-20', 5, 7),
...
--
-- Estructura Stand-in para la vista `vista_movimiento_entrada`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_movimiento_entrada` (
`cod_tipomov` int(11)
...
--
-- Estructura Stand-in para la vista `vista_movimiento_factura`
-- (Véase abajo para la vista actual)
--
CREATE TABLE `vista_movimiento_factura` (
`cod_tipomov` int(11)
,`nombre_mov` varchar(25)
,`cliente_cod` int(11)
,`cod_factura` int(11)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_cargo_empleado`  AS SELECT `t1`.`nombre_cargo` AS `nombre_cargo`, `t1`.`salario` AS `salario`, `t2`.`nombre1` AS `nombre1`, `t2`.`apellido2` AS `apellido2` FROM (`cargo` `t1` join `empleado` `t2`) WHERE `t1`.`cod_cargo` = `t2`.`cargo_cod` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_empleado_cliente`  AS SELECT concat(`t1`.`nombre1`,`t1`.`apellido1`) AS `empleado`, concat(`t2`.`nombre1`,`t2`.`nombre1`) AS `cliente` FROM (`empleado` `t1` join `cliente` `t2`) WHERE `t1`.`cod_empleado` = `t2`.`empleado_cod` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_movimiento_entrada`
--
DROP TABLE IF EXISTS `vista_movimiento_entrada`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_movimiento_entrada`  AS SELECT `t1`.`cod_tipomov` AS `cod_tipomov`, `t1`.`nombre_mov` AS `nombre_mov`, `t2`.`cod_entrada` AS `cod_entrada`, `t2`.`empleado_cod` AS `empleado_cod`, `t2`.`forma_pago` AS `forma_pago`, `t2`.`tipomov_cod` AS `tipomov_cod` FROM (`tipomov` `t1` join `entrada_cabeza` `t2`) WHERE `t1`.`cod_tipomov` = `t2`.`tipomov_cod` ;

-- --------------------------------------------------------

--
-- Estructura para la vista `vista_movimiento_factura`
--
DROP TABLE IF EXISTS `vista_movimiento_factura`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_movimiento_factura`  AS SELECT `t1`.`cod_tipomov` AS `cod_tipomov`, `t1`.`nombre_mov` AS `nombre_mov`, `t2`.`cliente_cod` AS `cliente_cod`, `t2`.`cod_factura` AS `cod_factura` FROM (`tipomov` `t1` join `factura_cabeza` `t2`) WHERE `t1`.`cod_tipomov` = `t2`.`tipomov_cod` ;
